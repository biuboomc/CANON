...
class RayResourcePool(ResourcePool):
    ...
    def get_placement_groups(self, strategy="STRICT_PACK", name=None):
        ...
        for idx, bundles in enumerate(pg_scheme):
            ...
            print(f"Before {idx}th Resource usage: ", resource_usage)

            available_resources = ray.available_resources()
            print(f"Before {idx}th Available resources: ", available_resources)
            print("bundles: ", bundles, "idx: ", idx, "pg_name_prefix: ", pg_name_prefix + str(idx), "lifetime: ", lifetime)
            pg = placement_group(bundles=bundles, strategy=strategy, name=pg_name_prefix + str(idx), lifetime=lifetime)
            resource_usage = ray.cluster_resources()
            print(f"After {idx}th Resource usage: ", resource_usage)

            available_resources = ray.available_resources()
            print(f"After {idx}th Available resources: ", available_resources)
            pgs.append(pg)

        

        print("before pg ready ", pgs)

        ray.get([pg.ready() for pg in pgs])
        print("after pg ready", pgs)
