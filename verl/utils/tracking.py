...
class ValidationGenerationsLogger:
    ...
    def log_generations_to_wandb(self, samples, step):
        ...
        columns = ["step"] + sum([[f"input_{i+1}", f"output_{i+1}", f"score_{i+1}", f"verify_{i+1}", f"backtrack_{i+1}", f"subgoal_{i+1}"] for i in range(len(samples))], [])
    ...
    def log_generations_to_mlflow(self, samples, step):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                row_data = []
                for sample in samples:
                    data = {"input": sample[0], "output": sample[1], "score": sample[2], "verify": sample[3], "backtrack": sample[4], "subgoal": sample[5]}
